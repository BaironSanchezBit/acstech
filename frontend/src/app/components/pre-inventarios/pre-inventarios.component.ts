...
@Component({
  selector: 'app-pre-inventarios',
  templateUrl: './pre-inventarios.component.html',
  styleUrl: './pre-inventarios.component.css'
})
export class PreInventariosComponent implements OnInit, OnDestroy {

  filteredLugares: Observable<{ name: string; }[]> | undefined; // Observable para las opciones filtradas
  ...
  tramitesIngresoForm: FormGroup;
  tramitesSalidaAutonalForm: FormGroup;
  ...
  formaPagoCompraForm: FormGroup;
  peritajeImprontasForm: FormGroup;
  documentosTraspasoForm: FormGroup;
  archivoDigitalForm: FormGroup;
  documentosValoresInicialesForm: FormGroup;
  deudaFinancieraForm: FormGroup;
  obsFase3Form: FormGroup;
  controlAccesoriosForm: FormGroup;
  formasPagoForm: FormGroup;
  liquidacionesForm: FormGroup;
  generadorContratosForm: FormGroup;
  tramitesForm: FormGroup;
  provicionTramitesForm: FormGroup;
  variablesLiquidacionForm: FormGroup;
  ...
  constructor(private modalService: NgbModal, private fasecoldaService: FasecoldaService, private conversionService: ConversionService, private sharedData: SharedDataService, private router: Router, private datePipe: DatePipe, private cdr: ChangeDetectorRef, private currencyService: CurrencyService, private http: HttpClient, private vehiclesService: VehiclesService, private clientsService: ClientsService, private authService: AuthService, private formBuilder: FormBuilder) {
    ...
    this.clienteForm = new FormGroup({
      ...
      ciudadIdentificacion: new FormControl(''),
      direccionResidencia: new FormControl(''),
      ciudadResidencia: new FormControl(''),
      celularOne: new FormControl('', [Validators.pattern(/^[0-9]+$/)]),
    ...
    this.filtroBDForm = this.formBuilder.group({
      ...
      estadoInventario: ['ASIGNADO EN INICIALES', [Validators.required]], // Valor por defecto
      fechaIngreso: [''],
      ubicacion: [''],
      tallerProveedor: [''],
    ...
    this.controlAccesoriosForm = this.formBuilder.group({
      copiaLlave: [{ value: '', disabled: false },[Validators.required]],
      copiaLlaveObs: [{ value: '', disabled: false }],
      gato: [{ value: '', disabled: true }],
      gatoObs: [{ value: '', disabled: true }],
      llavePernos: [{ value: '', disabled: true }],
      llavePernosObs: [{ value: '', disabled: true }],
      copaSeguridad: [{ value: '', disabled: true }],
      copaSeguridadObs: [{ value: '', disabled: true }],
      tiroArrastre: [{ value: '', disabled: true }],
      tiroArrastreObs: [{ value: '', disabled: true }],
      historialMantenimiento: [{ value: '', disabled: true }],
      historialMantenimientoObs: [{ value: '', disabled: true }],
      manual: [{ value: '', disabled: true }],
      manualObs: [{ value: '', disabled: true }],
      palomera: [{ value: '', disabled: true }],
      palomeraObs: [{ value: '', disabled: true }],
      tapetes: [{ value: '', disabled: true }],
      tapetesObs: [{ value: '', disabled: true }],
      ...
      llantaRepuesto: [{ value: '', disabled: true }],
      llantaRepuestoObs: [{ value: '', disabled: true }],
      kitCarretera: [{ value: '', disabled: true }],
      kitCarreteraObs: [{ value: '', disabled: true }],
      antena: [{ value: '', disabled: true }],
      antenaObs: [{ value: '', disabled: true }]
    });
  ...
  limpiarFormularios() {
    ...
    this.filtroBDForm.reset(
      { fechaIngreso: this.fechaActual,
        estadoInventario: 'ASIGNADO EN INICIALES', // Valor por defecto
       }
    );
  ...
  calcularTotalTramitesIngreso() {
    ...
    let comisionBruta = valorAutomagno - valorContrato;
    const formattedComisionBruta = this.formatSalary(comisionBruta);
  ...
  CreateDriveFolder(): Promise<string> {
    return new Promise((resolve, reject) => {
    console.log('Creando carpeta en Drive');
    const datos = {
      placa: this.vehiculo.placa,
      nombre: this.clients.primerNombre,
      apellido: this.clients.primerApellido
    };
  
    this.http.post(`${this.apiUrl}/api/create-folder`, datos, {}).subscribe(
      (respuesta) => {
        console.log('Respuesta del servidor:', respuesta);
        const resp: any = respuesta; resolve(resp.link || resp.folderId || respuesta);
      },
      (error) => {
  ...
  async createInventory() {
    ...
    this.linkDrive = await this.CreateDriveFolder();
    const linkInventarioSave = this.linkDrive;
    console.log('Link:', this.linkDrive);
    ...
    const elemento =
      this.vehiculo.marca + ' ' +
      this.vehiculo.linea + ' ' +
      this.vehiculo.version + ' ' +
      this.vehiculo.pasajeros + ' psj ' +
      this.vehiculo.modelo + ' ' +
      this.generadorContratosForm.get('kilometraje')?.value + ' kms placa ' +
      this.vehiculo.ciudadPlaca;
    ...
    //this.actaRecepcion();
    try {
      const response: any = await this.http.post(`${this.apiUrl}/api/preinventories`, inventoryData).toPromise();
      Swal.fire({
        position: "top-end",
        icon: "success",
        title: "Pre-Inventario creado correctamente",
      ...
      Swal.fire({
        title: "Error",
        text: "Hubo un error al crear el pre-inventario. Por favor, intenta nuevamente.",
        icon: "error",
  ...
  ngOnInit(): void {
    ...
    this.suscribirACambiosAutonal('retefuenteT');
    this.suscribirACambiosAutonal('garantiaMobiliaria');
    this.suscribirACambiosAutonal('impuestos');
    this.suscribirACambiosAutonal('soat');
    this.suscribirACambiosAutonal('revTecnoMeca');
    this.suscribirACambiosAutonal('comparendos');
    this.suscribirACambiosAutonal('documentacion');
    this.suscribirACambiosAutonal('manifiestoFactura');
    this.suscribirACambiosAutonal('semaforizacion');

    this.suscribirACambiosAutonalSalida('traspaso');
    this.suscribirACambiosAutonalSalida('servicioFueraBogota');
    this.suscribirACambiosAutonalSalida('retefuenteS');
    this.suscribirACambiosAutonalSalida('levantaPrenda');
    this.suscribirACambiosAutonalSalida('garantiaMobiliaria');
    this.suscribirACambiosAutonalSalida('impuestos');
    this.suscribirACambiosAutonalSalida('liquidacionImpuesto');
    this.suscribirACambiosAutonalSalida('derechosMunicipales');
    this.suscribirACambiosAutonalSalida('soat');
    this.suscribirACambiosAutonalSalida('revTecnoMeca');
    this.suscribirACambiosAutonalSalida('comparendos');
    this.suscribirACambiosAutonalSalida('documentosIniciales');
    this.suscribirACambiosAutonalSalida('documentacion');
    this.suscribirACambiosAutonalSalida('manifiestoFactura');
    this.suscribirACambiosAutonalSalida('ctci');
    this.suscribirACambiosAutonalSalida('semaforizacion');
    this.suscribirACambiosAutonalSalida('impuestoAnoActual');

    this.suscribirACambios('traspaso');
    this.suscribirACambios('retencion');
    this.suscribirACambios('otrosImpuestos');
    this.suscribirACambios('levantamientoPrenda');
    this.suscribirACambios('comparendos');
    this.suscribirACambios('proporcionalImpAnoCurso');
    this.suscribirACambios('devolucionSoat');
    this.suscribirACambios('honorariosAutomagno');

    this.suscribirACambios('retencionFuente');
    this.suscribirACambios('traspasoNeto');
    this.suscribirACambios('impuestoAnoCurso');
    this.suscribirACambios('otrosImpuestosProv');
    this.suscribirACambios('levantamientoPrenda2');
    this.suscribirACambios('comparendos2');
    this.suscribirACambios('deudaFinanciera');
    this.suscribirACambios('honorariosTramitador');
    ...
    this.http.get<any[]>(`${this.apiUrl}/api/getSuppliers`).subscribe(data => {
      this.suppliers = data.filter(supplier => supplier.tipoProveedor === "ABASTECIMIENTO");
      this.organizarAlfabeticamenteCliente();
    });

    this.http.get<any[]>(`${this.apiUrl}/api/getSuppliers`).subscribe(data => {
      this.tallerProveedor = data;
      this.organizarAlfabeticamenteCliente();
    });
  ...
  avanzarSiguiente() {
    ...
    // Aquí iría la lógica para cambiar de formulario o paso
  }
  ...
  contratoCompra() {
    if (this.filtroBDForm.get('proveedor')?.value !== 'AUTONAL' && this.clients.primerNombre !== 'AUTOMAGNO') {
      ...
      this.http.post(`${this.apiUrl}/api/contrato-compra`, dataParaDocumento, { responseType: 'blob' })
        .subscribe(blob => {
          Swal.close();
          const url = window.URL.createObjectURL(blob);
          const anchor = document.createElement('a');
          anchor.href = url;
          anchor.download = `CONTRATO DE COMPRA ${this.vehiculo.marca} ${this.vehiculo.linea} ${this.vehiculo.version} ${this.vehiculo.modelo} ${this.vehiculo.placa} [${numeroInventario}].docx`;
          anchor.click();
  ...
  contratoConsignacion() {
    if (this.filtroBDForm.get('proveedor')?.value !== 'AUTONAL' && this.clients.primerNombre !== 'AUTOMAGNO') {
      ...
      const valorCompraNumero = this.formaPagoCompraForm.get('valorCompraNumero')?.value;
      ...
      const dataParaDocumento = {
        ...
        valorCompraLetras: valorCompraLetras,
        generadorContratos: this.generadorContratosForm.getRawValue(),
        valorCompraNumero: valorCompraNumero,
        kilometraje: formattedKilometraje
      };
      ...
      this.http.post(`${this.apiUrl}/api/contrato-consignacion`, dataParaDocumento, { responseType: 'blob' })
        .subscribe(blob => {
          Swal.close();
          ...
          const anchor = document.createElement('a');
          anchor.href = url;
          anchor.download = `CONTRATO DE CONSIGNACIÓN ${this.vehiculo.marca} ${this.vehiculo.linea} ${this.vehiculo.version} ${this.vehiculo.modelo} ${this.vehiculo.placa} [${numeroInventario}].docx`;
          anchor.click();
          window.URL.revokeObjectURL(url);
        }, error => {
          Swal.close();
  ...
  actaRecepcion() {
    if (this.filtroBDForm.get('proveedor')?.value !== 'AUTONAL' && this.clients.primerNombre !== 'AUTOMAGNO') {
      ...
      const monthIndex = localTime.getMonth();
      const year = localTime.getFullYear();
      const formattedDate = `${day} de ${this.monthNames[monthIndex]} del ${year}`; const fechaFinSoat = this.documentosValoresInicialesForm.get('fechaFinSoat')?.value;
      ...
      this.http.post(`${this.apiUrl}/api/acta-recepcion`, dataParaDocumento, { responseType: 'blob' })
        .subscribe(blob => {
          ...
          const anchor = document.createElement('a');
          anchor.href = url;
          anchor.download = `ACTA DE RECEPCIÓN ${this.vehiculo.marca} ${this.vehiculo.linea} ${this.vehiculo.version} ${this.vehiculo.modelo} ${this.vehiculo.placa} [${numeroInventario}].docx`;
          anchor.click();
          window.URL.revokeObjectURL(url);
        }, error => {
          Swal.close();
  ...
  liquidacion() {
    ...
    const dataParaDocumento = {
      ...
      observacionesLiquidacionCompra: this.obsFase3Form.get('obsFase3')?.value,
  ...
  funt() {
    ...
    const diaImportacion = fechaImportacion.getDate();
    const mesImportacion = fechaImportacion.getMonth() + 1;
    ...
    if (this.buscarInventarioForm.valid) {
      ...
      this.http.get<any>(`${this.apiUrl}/api/getpreinventories/idpreinventories/${inventarioId}`).subscribe(
        data => {
          ...
          if (this.filtroBDForm.get('proveedor')?.value !== 'AUTONAL') {
            this.valorOfertado = this.formaPagoCompraForm.get('valorCompraNumero')?.value;
          } else {
            this.valorOfertado = this.tramitesIngresoForm.get('valorAutomagno')?.value;
          }
        ...
        error => {
          Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'Pre-Inventario no encontrado!',
          });
        }
      );
    } else {
      Swal.fire({
        icon: 'error',
        title: 'Oops...',
        text: 'Ingresa el numero del pre-inventario!',
      })
    }
  }
  ...
  async actualizarInventario() {
    ...
    try {
      let response: any = await this.http.put(`${this.apiUrl}/api/updatepreinventories/${this.inventoryId}`, updatedInventoryData).toPromise();
  ...
  onNextClick() {
    ...
    const modalActual = bootstrap.Modal.getInstance(document.getElementById('staticBackdrop3'));
    modalActual.hide();

    const modalSiguiente = new bootstrap.Modal(document.getElementById('staticBackdrop6'));
    modalSiguiente.show();
  }
